...
| **Gemini** | **YES (search-grounded inference; verified via id-field test)** | **roledef-instantiated; partial-by-default contract behavior** | **CONDITIONAL PASS — produced 2 of 6 contract entries by default, full 6 with explicit reinforcement; voice fidelity excellent throughout. See Gemini evidence file.** |
...
| **Grok Expert** | **YES** | **roledef-instantiated** | **PASS — voice, contract, workflow, pushback all conformant; see evidence file** |

The cross-runtime variance is now understood to be primarily a **roledef distribution** issue, not a roledef design issue. Runtimes split into three categories of behavior when handed a wrapper prompt with an unresolved URL placeholder:
...
The earlier "Claude failed conformance" finding from this session was incorrect — the test was on a chat surface that improvised, not on Claude Code which refused to improvise. **Claude Code's refusal-to-improvise is the right behavior** and simplifies the roledef-load skill's design: the skill's job is to provide the missing precondition (roledef content) that Claude Code is already correctly waiting for.
...
### PR #3 decision (`decisions/catdef-strategist.md`)
...
### PR #4 decision (`decisions/senior-jaded-vc-associate.md`)
...
This segmentation prevents the strategic mistake of viewing the roledef-load skill and the roledef.org distribution as substitutes. They are complements serving different beneficiaries.
...
2. Implement reference version that loads from `github.com/roledef-spec/roledef/roledefs/`
...
1. **Multi-axis conformance methodology.** The current methodology (in CLAUDE.md and SCHEMA.md) presumes single-axis conformance. Empirical evidence shows at least five axes: (i) roledef-content delivery (precondition), (ii) voice instantiation, (iii) workflow conformance, (iv) output_contract conformance, (v) conversation_rules conformance. Future schema/methodology work should formalize.
...
2. **Runtime classification (firm — four categories of valid loading + edge cases):**
   ...
   - **Explicit-fetch (wrapper-v2):** Claude Code (terminal/IDE). With wrapper-v2's explicit fetch instructions, calls WebFetch and loads roledef; full contract conformance.
   ...
   - Edge case — **Improvise-from-label-without-roledef (invalid):** chat surfaces with no fetch/grounding capability and given a passive placeholder. Produces output but the output measures runtime improvisation, not roledef conformance. Not valid evidence.
...
3. **roledef-read verification primitive.** The id-field discriminating test ("what's the roledef's `id` field? give only the literal value, no commentary") is empirically validated as the methodology's verification primitive. **Critical methodology rule surfaced this session:** never trust meta-question dodges as failure evidence. The roledef's "no meta-commentary on role/process" guardrail makes meta-questions about roledef loading status unreliable; only content questions (id field, specific guardrail text, output_contract entry names) can disambiguate roledef-loaded from improvising. Worth promoting to a SHOULD-pattern in SCHEMA.md or CLAUDE.md as the canonical verification protocol.

3a. **Contract-verification post-step.** For partial-default runtimes (Gemini-class), the roledef-load skill must include a post-bundle step that parses the runtime's bundle response against the roledef's output_contract entries and re-prompts for any missing entries by their roledef-spec names with their schema constraints. Empirically validated to recover Gemini-class runtimes to full conformance. Worth documenting as part of the wrapper-v3 / loading-skill specification.
...
5. **roledef size as a portability concern.** Multiple free-tier runtimes (Grok free, Perplexity free) refused the wrapper prompt + roledef reference on complexity grounds. Either roledef needs a "compact form" spec, or the standard accepts that some runtimes only test on paid tiers.
...
7. **`.openthing` extension portability (related to #6).** Some fetchers may filter on file extension. Once roledef.org serves roledefs, consider whether to ALSO serve at `.json` URLs (same content, dual extension via redirect or mirror) for fetchers that whitelist by file extension.
...
- Updated decision: [`./catdef-strategist.md`](./catdef-strategist.md) (30-day Turing target revised per this artifact)
- Updated decision: [`./senior-jaded-vc-associate.md`](./senior-jaded-vc-associate.md) (30-day Turing target partially fulfilled per this artifact)
- roledef: [`../roledefs/senior-jaded-vc-associate.openthing`](../roledefs/senior-jaded-vc-associate.openthing)
